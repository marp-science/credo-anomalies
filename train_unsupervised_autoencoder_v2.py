...
def build_unsupervised_dataset(data, labels, seed=42, kind='mnist'):
	...
	return np.vstack([validImagesAg]), np.vstack([anomalyImages]), np.vstack([validImages])
...
images, anomalies, originalSet = build_unsupervised_dataset(trainX, trainY, kind=args["kind"])
